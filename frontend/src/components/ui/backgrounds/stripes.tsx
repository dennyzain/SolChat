...
export function StripeGuides({
    ...
    const columns = useMemo(() => {
        const count = responsive ? Math.max(Math.floor(windowWidth / Number.parseInt(minColumnWidth)), 1) : columnCount
        return [...Array(count)]
    ...
    const lineColors = useMemo(() => {
        return {
            solid: darkMode ? "#333333" : "hsl(233 14.1% 96.1%)",
            dashed: darkMode ? "#2a2a2a" : "hsl(233 14% 93%)",
        }
    }, [darkMode])
                ...
                >
                    {columns.map((_, index) => (
                        <div key={index} className="relative h-full">
                            <div
                                className={`absolute inset-y-0 ${index === 0 ? "left-0" : index === columns.length - 1 ? "right-0" : "left-1/2"
